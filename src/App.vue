...
const guideWatching = ref(false)
const guideDistance = ref(null)
let watchId = null

const navOpen = computed(() => ['home', 'explore', 'saved'].includes(screen.value))
const canShare = computed(() => !!active.value)
...
function guideTo(place){
  ...
  guideWatching.value=true
  watchId=navigator.geolocation.watchPosition(({coords:c})=>{
    guideDistance.value=distanceKm(c.latitude,c.longitude,place.lat,place.lng)
  },()=>{}, {enableHighAccuracy:true,maximumAge:2000})
}
function stopGuide(){if(watchId!==null){navigator.geolocation.clearWatch(watchId);watchId=null}guideWatching.value=false}
...
<template>
  <div class="app-shell">
    <header v-if="!['guide'].includes(screen)" class="topbar">
    ...
    <main class="content">
      ...
      <section v-else-if="screen==='create'" class="flow-card">
        ...
        <button class="location-option exact" @click="precision='exact';askLocation"><span class="option-icon">⌖</span><span><b>Exact location</b><small>Pinpoint this place using GPS</small></span><span class="arrow">→</span></button>
      ...
      <section v-else-if="screen==='explore'" class="flow-card wide"><button class="back" @click="goHome">← Back</button><div class="section-kicker">EXPLORE</div><h2>Places around you.</h2><p>Open a saved Ciwaan or create a new one. Map discovery can be connected to your map provider when the production map key is configured.</p><button class="primary-cta full" @click="screen='saved'">My saved places <span>→</span></button></section>

      <section v-else-if="screen==='saved'" class="flow-card wide"><button class="back" @click="screen='explore'">← Back</button><div class="section-kicker">SAVED</div><h2>Your places.</h2><div v-if="saved.length" class="saved-list"><button v-for="place in saved" :key="place.code" class="saved-row" @click="showSaved(place)"><span class="option-icon">⌖</span><span><b>{{place.label}}</b><small>{{place.code}} · {{place.precision}}</small></span><span>→</span></button></div><div v-else class="empty">No saved places yet.</div></section>
    ...
    <nav v-if="navOpen" class="bottom-dock"><button :class="{active:screen==='home'}" @click="goHome"><span>⌂</span><small>Home</small></button><button class="create-fab" @click="openCreate" aria-label="Create Ciwaan">＋</button><button :class="{active:screen==='explore'||screen==='saved'}" @click="screen='explore'"><span>⌖</span><small>Explore</small></button></nav>
